# This script replicates and extends Jason Furman, Melissa Kearney, and Wilson Powell's analysis of childcare and employment - https://www.piie.com/blogs/realtime-economic-issues-watch/how-much-have-childcare-challenges-slowed-us-jobs-market

# They calculate the % change in employment to population ratio between Q1-2020 and Q1-2021 for men and women with and without a bachelors degree and with at least 1 child less than 13 years old (who needs childcare) compared to all other adults. They then do a counterfactual analysis that asks how the total YOY employment rate for men and women would change if parents with kids less than 13 had the employment rates of all other adults.

# One potential shortcoming here is that all other adults might not be the right counterfactual, but maybe parents of kids 13 and over would be a better counterfactual as we are still looking at parents, but parents that don't have to provide childcare.

# This script replicates their first graph and then shows how it would be different if instead of using all adults as a comparison, we use parents with kids over 13 as a comparison.

# It then replicates their counterfactual analysis in table 1 for employment-population ratio changes. The outcome doesn't EXACTLY match their results, but it's very close. I then redo the counterfactual, but use the employment-population ratio changes for parents with kids over 13 as the counterfactual. 
...
# The authors create age-sex-education cells and then calculate how different the employment-population ratio would be if parents of young kids had the same change in employment rates as all other adults within each cell.
...
gt:: gtsave(data=replicate_results_table, filename = 'replication_results.png')

# The results of this analysis match very closely, though not exactly with what FKP reported in their analysis. Basically, childcare seems to have very little impact on changes in epop rates because the difference in employment rates between parents of young kids and everyone else is pretty small and because the population of employed parents with young kids is pretty small, so assuming higher employment rates on this group has little impact on total epop changes of men and women.

### A revised counterfactual using parents of older kids

# This does the same thing as above, but breaks out adults into parents of young kids, older kids, and no kids and assumes the counterfactual where the parents of young kids have the same YOY epop ratio change as the parents of old kids (instead of all othe adults). The problem now is that some of the cells are pretty small. In particular the 16-24 year old cell with kids older than 13 is really small (a 16 year old can't have a kid over 13 years old), so epop changes are huge (100% decrease in one case) and does not reflect reality. So for this analysis I remove the 16-24 age group, and then run the revised counterfactual exercise. 
...
gt:: gtsave(data=revised_replicate_results_table, filename = 'revised_replication_results.png')
